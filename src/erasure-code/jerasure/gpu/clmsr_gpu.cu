...
subSubChunkStart(subSubChunkStart_), subSubChunkSize(subSubChunkSize_), clmsrProfileP(&(ClmsrGpuP_->clmsrProfile))\
{
    ...
    CUDA_CHECK_RETURN(cudaMalloc(&matrix_gpu, sizeof(int)*clmsrProfileP->k*clmsrProfileP->m));
    CUDA_CHECK_RETURN(cudaMemcpy(matrix_gpu, clmsrProfileP->matrix, sizeof(int)*(clmsrProfileP->k*clmsrProfileP->m), cudaMemcpyHostToDevice));
...
__global__ void pieceKernelGamma( gf_w8_log_gpu gf_table, unsigned char gamma, unsigned char** dataPt,  int nodeId, int calType, int dataSize, int patchSize, int w )
{
    ...
    for( int i = idx; i < dataSize; i += threadsNum )
    {
        ...
        dest[0][i]= A[0][i] * A[1][i];
        dest[1][i]= A[0][i] * A[1][i];
    }
}
    ...
    int* dm_ids, int* erasure_loc_data, int erased_data_size, int* erasure_loc_coding, int erased_coding_size, unsigned char** dataPt, int dataSize, int patchSize )
{
    ...
    if( idx == 0 )
    {
        printf( "66666666666666:::: %d, %d, %d, %d\n\n\n", dataSize, threadsNum, dataSize/threadsNum, k*erased_data_size*dataSize/threadsNum + k*erased_coding_size*dataSize/threadsNum );
    }
        ...
        for( int j = 0; j < erased_data_size; j ++ )
        {
            ...
            for( int t = idx; t < dataSize; t += threadsNum )
            {
                if( i == 0 )
                {
                    ...
                    erasure_now[t] = erasure_now[t] * 10;
                }
                else
                {
                    //todo: wrong test
            ...
            {//this is a hole (lost node)
                ...
                if( repaired_data.find(node_sw) != repaired_data.end())
                {
                    ...
                    {//recover both A1 and A2 here
                        ...
                        //pieceKernelGamma<<<pieceKernelGridSize,pieceKernelBlockSize,0,streams[erasure_locations[i]]>>>( gf_table, clmsrProfileP->gamma,planePOnGpuK,erasure_locations[i],GAMMA_INVERSE, pieceSize, pieceSizeMax, clmsrProfileP->w);
